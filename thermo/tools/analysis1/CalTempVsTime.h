...
#ifndef CalTempVsTime_h
#define CalTempVsTime_h
...
public:
  ...
  UInt_t getMinTime() const { return minTime_; }
  UInt_t getMaxTime() const { return maxTime_; }
  Float_t getCal(int idx) const { return cal_[idx]; }
  void read(std::ifstream& ofile);
...
class CalTempVsTime : public TSelector {
public :
  ...
  CalTempVsTime(TTree * /*tree*/ =0) : fChain(0) { }
  virtual ~CalTempVsTime() { }
  ...
  TList calibrationSets_;
  
  ClassDef(CalTempVsTime,0);
};

#endif

#ifdef CalTempVsTime_cxx
void CalTempVsTime::Init(TTree *tree)
{
