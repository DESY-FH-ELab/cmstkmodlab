...
class CalibrationRange : public TObject
{
public:
  CalibrationRange(UInt_t minTime, UInt_t maxTime);
  
  UInt_t getMinTime() const { return minTime_; }
  UInt_t getMaxTime() const { return maxTime_; }
  void addCalEntry(UInt_t channel, Float_t value);
  void dump(std::ofstream& ofile, UInt_t minTime, UInt_t maxTime);
