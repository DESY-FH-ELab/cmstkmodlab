...
int main(int argc, char *argv[])
{
  ...
  QCoreApplication::setApplicationName("thermoDAQ2Log");
  ...
  if (version=="1") {
    ThermoDAQStreamReader *reader = new ThermoDAQStreamReader(filename, &app);
    QObject::connect(reader, SIGNAL(finished()), &app, SLOT(quit()));
    QTimer::singleShot(0, reader, SLOT(run()));

  } else if (version=="2") {
    ThermoDAQ2StreamReader *reader = new ThermoDAQ2StreamReader(filename, &app);
    QObject::connect(reader, SIGNAL(finished()), &app, SLOT(quit()));
    QTimer::singleShot(0, reader, SLOT(run()));
