...
{
  NQLog("ConradManager") << ": updating channel " << channelNumber << " status to " << ConradModel_ -> getSwitchState(chNumber);
  emit updateVacuumChannelState(channelNumber - 1, ConradModel_ -> getSwitchState(chNumber));
}
