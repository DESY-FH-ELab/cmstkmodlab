    ...
    QWidget(parent), model_(model)
{
  ...
  operationLayout->addWidget(new LeyboldGraphixOneGaugeWidget(model_, this));
   ...
   model_(model)
{
  ...
  statusDisplay_ = new QLabel(model_->getSensorStatusText(VLeyboldGraphix::SensorStatus_nosen).c_str(),
                              this);
...
void LeyboldGraphixOneGaugeWidget::updateInfo()
{
  ...
  status = model_->getSensorStatus();
  pressure = model_->getPressure();

  statusDisplay_->setText(model_->getSensorStatusText(status).c_str());
