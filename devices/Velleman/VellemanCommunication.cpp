...
bool VellemanCommunication::initialize()
{
  ...
  if (m_ioPort < 0) {
    ...
    // NOTE: use Linux command "errno <number>" to find errno meaning
    return false;
  }
...
{ 
  ...
  buffer[3] = param1;
  buffer[4] = param2;
  buffer[5] = ~(STX + cmd + mask + param1 + param2) + 1;
  buffer[6] = ETX;
...
bool VellemanCommunication::receiveAnswer(unsigned char* cmd, unsigned char* mask, unsigned char* param1, unsigned char* param2) const
{
  ...
  // check for correct checksum byte (6/7)
  int sum = 0;
  for (int i = 0; i < 5; i++)
    sum += buffer[i];
  int checksum = ~(sum) + 1;
