...
bool VellemanControllerFake::setRelay(unsigned relay, unsigned relayCMD) const
{
  if (relay > 8) {
...
bool VellemanControllerFake::setSingleRelay(unsigned relay, unsigned relayCMD) const
{
  if (relay > 8) {
    fprintf(stderr, "[Velleman] ERROR (port FAKE): '%u' is out of range of relays 1-8 available for Velleman relay card, ERRNO: %i\n", relay, errno);
    return false;
...
//! Set all relays (those listed in input are turned on, all others off)
bool VellemanControllerFake::setMultiRelays(std::vector<unsigned> relays) const
{
  ...
  for (int f=0; f<8; ++f) status_[f] = false;
