...
///
///
DefoConfigWriter::DefoConfigWriter( const std::string & outputFileName ) {
...
///
///
DefoConfigWriter::~DefoConfigWriter() {

}


void DefoConfigWriter::write(std::map<std::string,std::string> &keyvalueMap) {
...
void DefoConfigWriter::writeMerge(std::map<std::string,std::string> &keyvalueMap) {
  ...
  std::ifstream file( outputFileName_.c_str(), std::ios::in );
  if( !file.good() ) {
    std::cerr << " [DefoConfigWriter::openAndCheckFile] ** ERROR: failed to open file: "
	      << outputFileName_ << "." << std::endl;
    QMessageBox::critical( 0, tr("[DefoConfigWriter::fill]"),
...
void DefoConfigWriter::writeNew(std::map<std::string,std::string> &keyvalueMap) {
